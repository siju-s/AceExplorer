...
class FileDataFetcher : DataFetcher {
    ...
    override fun fetchData(context: Context, path: String?, category: Category): ArrayList<FileInfo> {
        return fetchFiles(path, getSortMode(context), canShowHiddenFiles(context), RootUtils.isRooted(context))
    }
    ...
    private fun fetchFiles(path: String?, sortMode: Int, showHidden: Boolean, isRooted: Boolean): ArrayList<FileInfo> {
        if (path == null) {
    ...
    private fun getFilesList(path: String, root: Boolean,
    ...
    private fun getFilesList(listFiles: Array<File>?, showHidden: Boolean): ArrayList<FileInfo> {
