...
class LargeImageFilesFetcher : DataFetcher {

    override fun fetchData(context: Context, path: String?, category: Category): ArrayList<FileInfo> {
        val showHidden = canShowHiddenFiles(context)
        val cursor = fetchLargeImageFiles(context, showHidden)
    ...
    override fun fetchCount(context: Context, path: String?): Int {
        val cursor = fetchLargeImageFiles(context, canShowHiddenFiles(context))
        return getCursorCount(cursor)
    }

    private fun fetchLargeImageFiles(context: Context, showHidden: Boolean): Cursor? {
        val uri = MediaStore.Files.getContentUri("external")
        var selection = ""
        if (!showHidden) {
            selection = constructionNoHiddenFilesArgs() + " AND "
        }
        selection += DocumentUtils.getMediaStoreImageMediaType() + " AND " + MediaStore.Files.FileColumns.SIZE + " >?"
