...
class LargeDocFilesFetcher : DataFetcher {

    override fun fetchData(context: Context, path: String?, category: Category): ArrayList<FileInfo> {
        val showHidden = canShowHiddenFiles(context)
        val cursor = fetchLargeDocs(context, showHidden)
    ...
    override fun fetchCount(context: Context, path: String?): Int {
        val cursor = fetchLargeDocs(context, canShowHiddenFiles(context))
        return getCursorCount(cursor)
    }

    private fun fetchLargeDocs(context: Context, showHidden: Boolean): Cursor? {
        val uri = MediaStore.Files.getContentUri("external")
        var selection = ""
        if (!showHidden) {
            selection = constructionNoHiddenFilesArgs() + " AND "
        }
        selection += DocumentUtils.getDocMimeTypes() + " AND " + MediaStore.Files.FileColumns.SIZE + " >?"
