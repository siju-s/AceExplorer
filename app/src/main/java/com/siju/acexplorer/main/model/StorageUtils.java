...
public class StorageUtils {
    ...
    public static List<String> getStorageDirectories() {
        ...
        if (TextUtils.isEmpty(rawEmulatedStorageTarget)) {
            ...
            if (TextUtils.isEmpty(rawExternalStorage)) {
                ...
                paths.add(StoragesGroup.STORAGE_SDCARD0);
            } else {
    ...
    private static String[] getExtSdCardPaths() {
        ...
        for (File file : context.getExternalFilesDirs("external")) {
            if (file != null) {
                int index = file.getAbsolutePath().lastIndexOf(StoragesGroup.ANDROID_DATA);
                if (index < 0) {
        ...
        File file = new File(StoragesGroup.STORAGE_SDCARD1);
        if (file.exists() && file.canExecute() && !paths.contains(file.getAbsolutePath())) {
            paths.add(StoragesGroup.STORAGE_SDCARD1);
        }
